...
def process_db(file_name): # (+ liste des noms des éléments à récupérer) / renommer col1, col2 par nom en-tête / file de HPA dispo dans ce dossier et datant de mars 2024.
    ...
    with open(file_name, 'r') as filin:
    	...
    	for ligne in csvreader:
    		...
    		col3 = ligne[2].split('!') if ligne[2] else ["Not specified"] # geneName
    		col4 = ligne[3].split('!') if ligne[3] else ["Not specified"] # diseaseId
    		col5 = ligne[4].split('!') if ligne[4] else ["Not specified"] # diseaseName
						...
						if re.match(r'\b' + re.escape(event.lower()) + r'\b(?![-])', w.lower()):
							...
							if genes not in remove_name :
								...
								data.append({
											...
											"geneSymbol": slist[0],
											"geneName": ','.join(slist[1:2]), 
											"diseaseId": ','.join(slist[2:3]), 
											"diseaseName": ','.join(slist[3:4])
										})
