...
def process_db(file_name): # (+ liste des noms des éléments à récupérer) / renommer col1, col2 par nom en-tête / file de HPA dispo dans ce dossier et datant de mars 2024.
    ...
    with open(file_name, 'r') as filin:
    	...
    	for ligne in csvreader:
    		if not any('ICD10' in item for item in ligne):
    			col2 = ligne[1].split('!') if ligne[1] else ["Not specified"] # geneSymbol
    			col3 = ligne[2].split('!') if ligne[2] else ["Not specified"] # diseaseId
    			col4 = ligne[3].split('!') if ligne[3] else ["Not specified"] # diseaseName
    			col7 = ligne[6].split('!') if ligne[6] else ["Not specified"] # confidence score

    			columns = col2 + col3 + col4 + col7
			...
			for gene in sublist_gene:
				...
				for word in pro_list:
					if gene.lower() == word.lower():
						...
						gene_symbol = [g for g in genes_db if g in sublist_gene]
